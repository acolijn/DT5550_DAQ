...
def main(argv):
    ...
    while ievent < n_event:
        if not OSCILLOSCOPE_Oscilloscope_0_START(handle):
            print("Start Error")
        # start reading the scope
            ...
            while (Oscilloscope_Status != 1):
                [_, Oscilloscope_Status] = OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle)
                print('Status waiting for trigger ...', Oscilloscope_Status)
            # scope finds a trigegr at a certain location in the circular buffer
            [_, Event_Position] = OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle)
            # get the data.....
            [_, Oscilloscope_Data, _, _] = OSCILLOSCOPE_Oscilloscope_0_GET_DATA(Timeout_ms, handle)
